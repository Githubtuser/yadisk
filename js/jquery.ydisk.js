...
;(function (window, document, $, undefined) {
    ...
    ajaxQueue = function( ajaxOpts ) {
        ...
        ydiskQueue.queue( doRequest );  //Метод .queue() позволяет нам назначать ряд функций, которые должны быть применены.
                                        // В .queue() можно передать функцию обратного действия; это позволяет нам добавлять метод в конец очереди.
                                        // Функция обратного действия запускается единожды для каждого элемента.
    ...
    $.fn.ydisk = function(options) {
    	...
    	options = $.extend( true, {
                ...
                replacer      : {
                    ...
                    video : '<video id="{id}" class="ydisk-video" controls autoplay  src="{href}"{size}></video>',// добавил poster="Cover.jpg" poster="Cover.jpg" controls
